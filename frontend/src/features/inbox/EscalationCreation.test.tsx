...
describe('frontend escalation creation', () => {
  ...
  it('creates once on a double submit, sends only the escalation payload, refreshes detail, and shows the authoritative result', async () => {
    ...
    const messageHistory = screen.getByRole('region', { name: 'Message history' })
    expect(messageHistory).toHaveAttribute('tabindex', '0')
    expect(within(messageHistory).getByText(messageFixture().text!)).toBeInTheDocument()
