...
def test_policy_is_explicitly_versioned_and_contains_authority_limits():
    ...
    assert AI_SYSTEM_POLICY_VERSION == "mbb-ai-policy-v3"
    ...
    structured = get_system_policy("french", structured_commercial=True)
    assert "output exactly one JSON object" in structured.text
    assert "Use sellable_item_id references only" in structured.text
    assert "output exactly one JSON object" not in policy.text
